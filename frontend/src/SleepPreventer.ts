export class SleepPreventer {
	private noSleep: boolean = false

	private fakeVideo: HTMLVideoElement | null = null

	private static readonly MP4_SRC =
		'data:video/mp4;base64,'

	private static readonly WEBM_SRC =
		'data:video/webm;base64,'

	public constructor() {
		this.fakeVideo = document.createElement('video')
		this.fakeVideo.setAttribute('playsinline', '')

		let source = document.createElement('source')
		source.src = SleepPreventer.MP4_SRC
		source.type = 'video/mp4'
		this.fakeVideo.append(source)

		source = document.createElement('source')
		source.src = SleepPreventer.WEBM_SRC
		source.type = 'video/webm'
		this.fakeVideo.append(source)
	}

	public activate(): void {
		if (!this.noSleep) {
			this.noSleep = true
			this.keepAwake()
		}
	}

	public deactivate(): void {
		if (this.noSleep) {
			this.noSleep = false
		}
	}

	private async keepAwake(): Promise<void> {
		while (this.noSleep) {
			if (this.fakeVideo) {
				this.fakeVideo.play()
			}
			await new Promise((resolve) => setTimeout(resolve, 10_000))
		}
	}
}

export const sleepPreventer = new SleepPreventer()
